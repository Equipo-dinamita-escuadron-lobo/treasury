...
class SignedJwtAuthenticationTest {
    ...
    @Test void validSignedStudentTokenIsAuthenticatedAndAuthorized()throws Exception{when(queries.search(any())).thenReturn(new PageResult<>(java.util.List.of(),0,0,0,20));mvc.perform(get("/api/treasury/payment-vouchers").param("enterpriseId","ent").header("Authorization","Bearer "+token("Estudiante"))).andExpect(status().isOk());}
    ...
    @Test void tamperedTokenIsRejected()throws Exception{String token=token("Estudiante");mvc.perform(get("/api/treasury/payment-vouchers").param("enterpriseId","ent").header("Authorization","Bearer "+token.substring(0,token.length()-2)+"xx")).andExpect(status().isUnauthorized());}
