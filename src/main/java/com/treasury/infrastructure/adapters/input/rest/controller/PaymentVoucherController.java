...
public class PaymentVoucherController {
    ...
    @PostMapping public VoucherResponse create(@Valid @RequestBody VoucherRequest request){return mapper.toResponse(commands.create(mapper.toCommand(request)));}
    @PutMapping("/{id}") public VoucherResponse update(@PathVariable Long id,@Valid @RequestBody VoucherRequest request){return mapper.toResponse(commands.update(id,mapper.toCommand(request)));}
    ...
    @PostMapping("/{id}/post") public VoucherResponse post(@PathVariable Long id,@RequestParam String enterpriseId,@RequestHeader("Idempotency-Key") String key){return mapper.toResponse(commands.post(id,enterpriseId,key));}
    @PostMapping("/{id}/void") public VoucherResponse voidVoucher(@PathVariable Long id,@RequestParam String enterpriseId,@Valid @RequestBody VoidRequest request){return mapper.toResponse(commands.voidVoucher(id,enterpriseId,request.reason()));}
    @GetMapping("/{id}") public VoucherResponse find(@PathVariable Long id,@RequestParam String enterpriseId){return mapper.toResponse(queries.find(id,enterpriseId));}
